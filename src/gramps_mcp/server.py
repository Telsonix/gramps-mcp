...
TOOL_REGISTRY: Dict[str, Dict[str, Any]] = {
    ...
    "find_type": {
        "description": (
            ...
            "2. Persons do NOT have a 'birth' property. Birth/death are events in event_ref_list. "
            "3. Array indexing [N] CANNOT be combined with .any/.all traversal in the same chain. "
            "   e.g. event_ref_list.any.ref.get_event.date.dateval[2] < 1950 FAILS (422). "
            "   For dates, search type=event directly, then look up people from results. "
            "CORRECT GQL EXAMPLES: "
            "- Surname: primary_name.surname_list[0].surname = \"Smith\" "
            "- First name: primary_name.first_name ~ \"John\" "
            "- Any surname contains: primary_name.surname_list.any.surname ~ \"Smith\" "
            "- Gender (0=female, 1=male): gender = 1 "
            "- Has media: media_list "
            "- Private record: private "
            "- Family has children: child_ref_list.length > 0 "
            "- Person linked to event with keyword: event_ref_list.any.ref.get_event.description ~ farmer "
            "- Events in a year range (type=event): date.dateval[2] > 1800 and date.dateval[2] < 1950 "
            "- Note contains text (type=note): text.string ~ \"Smith\" "
            "FOR BIRTH YEAR SEARCH: Use type=event with gql='date.dateval[2] < 1950', "
            "then call get_type on the returned person handles. "
            "WRONG (will fail): surname = x, birth.date, event_ref_list.any.ref.get_event.date.dateval[2] "
            "Returns list of matching records with handles and gramps_ids. "
            "Full GQL docs: gql://documentation or https://github.com/DavidMStraub/gramps-ql"
        ),
    ...
    "get_entity": {
        "description": (
            ...
            "and either handle (internal key) or gramps_id (e.g. 'I0001', 'F0001', 'E0012'), not both. "
            "Returns full record details using the appropriate formatter for each type: "
            "- person/family: names, relationships, all events, media, notes, citations "
            "- event: date, place, description, participants, citations "
            "- place: name, location, coordinates, hierarchy "
            "- source: title, author, publication info, citations "
            "- citation: source reference, page, date, confidence "
            "- media: description, file path, MIME type, linked objects "
            "- note: full text content and type "
            "- repository: name, type, address, linked sources "
    ...
    "create_person": {
        "description": (
            ...
            "NAME FORMATS: "
            "- Plain string: last word becomes surname (e.g. 'John Smith'). "
            "- Multiple surnames: use '|' separator (e.g. 'Sergey | Andreev | fon Venberg'). "
            "- With name type: append '(Type)' (e.g. 'Jane Smith (Married Name)'). "
            "- Full dict: {'first_name': 'John', 'surname_list': ['Smith', 'Jones'], 'type': 'Birth Name'}. "
            "ALTERNATE NAMES: accepts a list of name strings or dicts, OR a plain string "
            "that is split on commas (e.g. 'Jane Smith (Married Name), Jane Doe'). "
            "Use get_types to see all valid name types (listed under 'Name Types'). "
